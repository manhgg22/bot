...
export function calcRSI(candles, period = 14) {
  ...
  // Bắt đầu từ nến thứ `period`
  for (let i = period; i < candles.length; i++) {
    // Tính RSI cho nến hiện tại
    const rs = avgLoss === 0 ? 100 : avgGain / avgLoss;
    rsiArr.push(100 - 100 / (1 + rs));

    // Cập nhật avgGain và avgLoss cho nến tiếp theo
    const diff = candles[i].close - candles[i - 1].close;
    if (diff >= 0) {
...
/* ============== CÁC HÀM PHÂN TÍCH CHO SMC (GIỮ NGUYÊN) ============== */
...
async function getLTFEntry(symbol, bias) {
  ...
  let sl, tp;
  const ATR_SL = 1.5;   // SL = 1.5 ATR
  const ATR_TP = 3.0;   // TP = 3 ATR (RR 1:2)
  if (bias === "BULLISH") {
...
/* ============== [CHIẾN LƯỢC 1] Tín hiệu Confluence SMC ============== */
export async function getSignalsSMC(symbol) {
  ...
  return {
    strategy: "SMC", // Đánh dấu chiến lược
    direction,
    dailyBias, bosH1, bosH4, price: ltf.price, rsi: ltf.rsi,
    ob: ltf.ob, fvg: ltf.fvg, retestOb: ltf.retestOb, retestFvg: ltf.retestFvg,
    atr: ltf.atr, tp: ltf.tp, sl: ltf.sl
  };
}
...
export async function getSignalsEMACross(symbol) {
    ...
    if (candles.length < 201) {
        return { direction: "NONE", reason: "Not enough H1 candles for EMA Cross" };
    }
    ...
    if (isLongSignal || isShortSignal) {
        ...
        if (!atr) return { direction: "NONE", reason: "ATR calculation failed" };
        
        const direction = isLongSignal ? "LONG" : "SHORT";
        const sl = isLongSignal ? lastClose - 2 * atr : lastClose + 2 * atr;
        const tp = isLongSignal ? lastClose + 4 * atr : lastClose - 4 * atr; // RR 1:2

        return {
            strategy: "EMA_CROSS", // Đánh dấu chiến lược
            direction,
            price: lastClose,
            tp,
            sl
